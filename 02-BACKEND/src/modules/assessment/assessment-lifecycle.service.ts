...
import { buildDefinitionResponse } from './assessment-definition-view';
...
export class AssessmentLifecycleService {
  ...
  async saveAnswer(
    ...
    if (assessment.state === 'NOT_STARTED') {
      ...
      await this.transitionOnboarding(userId, 'ASSESSMENT_IN_PROGRESS', now);
    } else {
      await this.prisma.assessment.update({
        where: { id: assessment.id },
  ...
  private async transitionOnboarding(userId: string, target: string, now: Date): Promise<void> {
    ...
    if (!existing) return;
    const pre = ['ASSESSMENT_PENDING', 'ASSESSMENT_IN_PROGRESS'];
