...
import { ASSESSMENT_DEFINITION_VERSION } from './assessment-definition';
import { type SubmitResponse } from './assessment.dto';
import {
...
import { presentResult, type ResultInsight } from './result-presenter';
...
import {
  ...
  toResultResponse,
} from './assessment-result-mapping';
...
export class AssessmentSubmitService {
  ...
  async submit(userId: string): Promise<SubmitResponse> {
    ...
    await this.transitionOnboarding(userId, 'COMPLETED', now);
  ...
  private async transitionOnboarding(userId: string, target: string, now: Date): Promise<void> {
    const existing = await this.prisma.onboardingState.findFirst({ where: { userId } });
    if (!existing) return;
    const pre = ['ASSESSMENT_IN_PROGRESS', 'ASSESSMENT_SUBMITTED'];
    const next = pre.includes(existing.state) ? target : existing.state;
  ...
  private async assertCanEnter(userId: string): Promise<void> {
