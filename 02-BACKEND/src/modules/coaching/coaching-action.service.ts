...
import { recomputePlanStatus } from './coaching-lifecycle';
import { ActionConflictException, ActionResultNotFoundException, PlanNotActiveException, PlanNotReadyException } from './coaching.errors';
import { CoachingEligibilityService } from './coaching-eligibility.service';
import type { UpdateActionInput, UpdateActionResponse } from './coaching.dto';
...
export class CoachingActionService {
  ...
  async updateAction(userId: string, actionId: string, input: UpdateActionInput): Promise<UpdateActionResponse> {
