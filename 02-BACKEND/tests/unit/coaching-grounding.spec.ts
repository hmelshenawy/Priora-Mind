...
import { CoachingGroundingService, buildFocusAreaEvidence } from '../../src/modules/coaching/coaching-grounding.service';
import { PlanUnavailableException } from '../../src/modules/coaching/coaching.errors';
...
function prisma(overrides: { library?: unknown; disclaimer?: unknown } = {}) {
  return {
    coachingActionLibrary: {
      findUnique: vi.fn().mockResolvedValue(overrides.library ?? {
        content: COACHING_LIBRARY_V1.content,
        integrity: COACHING_LIBRARY_V1.integrity,
      }),
    },
    coachingDisclaimer: {
      findUnique: vi.fn().mockResolvedValue(overrides.disclaimer ?? {
...
describe('CoachingGroundingService', () => {
  ...
  it('looks up pinned snapshots by exact version and no active flag', async () => {
    const db = prisma();
    await expect(new CoachingGroundingService(db as never).assemble(scoredResult)).rejects.toBeInstanceOf(PlanUnavailableException);
